...
// ============================================
// UPDATED LOGIN ENDPOINT - FIXED!
// ============================================
authRouter.post('/login', validateFields(['email', 'password']), async (req, res) => {
    try {
        const { email, password } = req.body;

        // Verify password using Firebase REST API
        const firebaseUser = await verifyPassword(email, password);
        if (!firebaseUser) {
        ...
        const customToken = await auth.createCustomToken(firebaseUser.localId, {
            role: userData.role,
            email: userData.email
        });

        // Update last login
        await db.collection('users').doc(firebaseUser.localId).update({
