<?php
// getStaffImage.php - Handles staff image retrieval with fallback to SVG placeholders
...
if (is_dir($uploadPath)) {
    $files = glob($uploadPath . "*_rank_{$rank}_*.{jpg,jpeg,png,gif,webp}", GLOB_BRACE);
    if (!empty($files)) {
        $imageFile = $files[0];
    }
}
...
if ($imageFile && file_exists($imageFile) && !$preview) {
    ...
    $svg .= '<text x="200" y="200" font-family="Arial" font-size="120" fill="' . $textColor . '" text-anchor="middle" dy=".3em">' . $displayInitials . '</text>';
    $svg .= '</svg>';
