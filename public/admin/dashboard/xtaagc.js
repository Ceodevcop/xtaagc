...
function initializeAllSections() {
    const sections = ['users', 'admins', 'investors', 'clients', 'events', 'testimonials', 'opportunities', 'announcements', 'audit'];
...
async function loadUsers() {
    try {
        ...
        document.getElementById('usersTableBody').innerHTML = html || '<tr><td colspan="8">No users found</td></tr>';
        
        if (dataTables.usersTable) {
            dataTables.usersTable.destroy();
        }
        dataTables.usersTable = $('#usersTable').DataTable({
            responsive: true,
            pageLength: 25,
            order: [[6, 'desc']]
        });
...
async function loadUsersByRole(role) {
    ...
    try {
        ...
        dataTables[tableId] = $(`#${tableId}Table`).DataTable({
            responsive: true,
            pageLength: 25
        });
...
async function loadEvents() {
    try {
        ...
        snapshot.forEach(doc => {
            ...
            html += `
                <tr>
                    ...
                    <td>
                        <div class="action-icons">
                            <i class="fas fa-edit" onclick="editEvent('${doc.id}')"></i>
                            <i class="fas fa-trash" onclick="openDeleteModal('event', '${doc.id}')"></i>
                        </div>
                    </td>
                </tr>
            `;
        ...
        document.getElementById('eventsTableBody').innerHTML = html || '<tr><td colspan="6">No events found</td></tr>';
        
        if (dataTables.eventsTable) {
            dataTables.eventsTable.destroy();
        }
        dataTables.eventsTable = $('#eventsTable').DataTable({
            responsive: true,
            pageLength: 25
        });
...
async function loadTestimonials() {
    try {
        ...
        snapshot.forEach(doc => {
            ...
            html += `
                <tr>
                    ...
                    <td>
                        <div class="action-icons">
                            <i class="fas fa-edit" onclick="editTestimonial('${doc.id}')"></i>
                            <i class="fas fa-trash" onclick="openDeleteModal('testimonial', '${doc.id}')"></i>
                        </div>
                    </td>
                </tr>
            `;
        ...
        document.getElementById('testimonialsTableBody').innerHTML = html || '<tr><td colspan="6">No testimonials found</td></tr>';
        
        if (dataTables.testimonialsTable) {
            dataTables.testimonialsTable.destroy();
        }
        dataTables.testimonialsTable = $('#testimonialsTable').DataTable({
            responsive: true,
            pageLength: 25
        });
...
async function loadOpportunities() {
    try {
        ...
        snapshot.forEach(doc => {
            ...
            html += `
                <tr>
                    ...
                    <td>
                        <div class="action-icons">
                            <i class="fas fa-edit" onclick="editOpportunity('${doc.id}')"></i>
                            <i class="fas fa-trash" onclick="openDeleteModal('opportunity', '${doc.id}')"></i>
                        </div>
                    </td>
                </tr>
            `;
        ...
        document.getElementById('opportunitiesTableBody').innerHTML = html || '<tr><td colspan="6">No opportunities found</td></tr>';
        
        if (dataTables.opportunitiesTable) {
            dataTables.opportunitiesTable.destroy();
        }
        dataTables.opportunitiesTable = $('#opportunitiesTable').DataTable({
            responsive: true,
            pageLength: 25
        });
...
async function loadAnnouncements() {
    try {
        ...
        snapshot.forEach(doc => {
            ...
            html += `
                <tr>
                    ...
                    <td>
                        <div class="action-icons">
                            <i class="fas fa-edit" onclick="editAnnouncement('${doc.id}')"></i>
                            <i class="fas fa-trash" onclick="openDeleteModal('announcement', '${doc.id}')"></i>
                        </div>
                    </td>
                </tr>
            `;
        ...
        document.getElementById('announcementsTableBody').innerHTML = html || '<tr><td colspan="6">No announcements found</td></tr>';
        
        if (dataTables.announcementsTable) {
            dataTables.announcementsTable.destroy();
        }
        dataTables.announcementsTable = $('#announcementsTable').DataTable({
            responsive: true,
            pageLength: 25
        });
...
async function loadAuditLogs() {
    try {
        ...
        document.getElementById('auditTableBody').innerHTML = html || '<tr><td colspan="5">No logs found</td></tr>';
        
        if (dataTables.auditTable) {
            dataTables.auditTable.destroy();
        }
        dataTables.auditTable = $('#auditTable').DataTable({
            responsive: true,
            pageLength: 50,
            order: [[0, 'desc']]
        });
...
async function loadBackupInfo() {
    try {
        ...
        document.getElementById('backupsTableBody').innerHTML = html || '<tr><td colspan="4">No backups found</td></tr>';
...
async function loadNotifications() {
    try {
        ...
        document.getElementById('notificationsList').innerHTML = html || '<div class="notification-item">No new notifications</div>';
...
async function deleteItem(type, id) {
    try {
        await db.collection(type === 'user' ? 'users' : type + 's').doc(id).delete();
